...
def create_app(config_name='development'):
    ...
    # Create database tables (only in development, or if explicitly enabled)
    # In production, use migrations instead of create_all()
    if config_name == 'development' or os.getenv('AUTO_CREATE_TABLES', 'false').lower() == 'true':
        try:
            ...
            import logging
            logging.warning(f"Failed to create database tables: {str(e)}")
            if config_name == 'development':
                raise  # In development, fail fast
